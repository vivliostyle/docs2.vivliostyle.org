...
- **[Vivliostyle CLI](/ja/cli/) v11.1.0** — 目次ナビゲーションの中身を組み立て直す [`toc.compose` オプション](/ja/cli/config/#tocconfig)を追加。[`renderMode: docker`](/ja/cli/special-output-settings/#docker-を利用した生成) は非推奨に。[プロジェクトの作成](/ja/cli/getting-started/#vivliostyle-プロジェクトを作成する)は `npm create book@latest` に統一し、必要な Node.js のバージョンを v22.12.0 以上に更新
