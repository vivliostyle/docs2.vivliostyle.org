...
このガイドでは、名前付きページと `:nth(An+B of C)` ページセレクタを解説します。これらを組み合わせることで、書籍の章ごとにまったく異なるページレイアウトを与えられるようになります。
...
`page` プロパティで、ブロックレベルの要素（section、`<div>` など）に名前を付けます:
...
ページ番号はデフォルトで1から始まりますが、フローボックスに `counter-reset: page <n>` を指定すると、ページカウンタを `<n>` から再開できます。これにより `:nth(1)` を*この章の最初のページ*の意味で使えます:
...
## 6. `:nth(An+B of C)` ―― ページグループ*内*のセレクタ（v2.39.0新機能）
...
このパターンは CSS GCPM 3 仕様の Example 13・14 で示されており、Vivliostyle.js v2.39.0 でネイティブにサポートされるようになりました。

## 8. Vivliostyle Viewer での確認方法
