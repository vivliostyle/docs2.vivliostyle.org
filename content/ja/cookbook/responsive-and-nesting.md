...
[CSS Nesting Module](https://www.w3.org/TR/css-nesting-1/) は、CSS 内で `&` を使ってルールを階層化する仕組みです。Sass を使ったことがあれば馴染みのある記法ですが、**ビルドステップを介さずブラウザ・Vivliostyle が直接解釈**します。
...
- **Sass からの段階移行**: `.scss` ファイルを `.css` にリネームして `@import` 系を削除するだけで、`&` ネストの大半はそのまま動きます。Sass 固有の `@mixin` / `@function` を使っている箇所だけ書き直してください。
