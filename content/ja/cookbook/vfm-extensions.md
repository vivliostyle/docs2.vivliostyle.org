---
title: VFM 拡張機能ガイド
description: VFM 2.7.0 で追加された captionlessImagePolicy・editPlugins フック・valibot プラグインオプションスキーマ
lang: ja
order: 5
---
...
VFM 2.7.0 では、外部から VFM の挙動を細かく制御するための拡張ポイントが 3 つ追加されました。
...
VFM 2.7.0 では `captionlessImagePolicy` が追加され、`alt` が空のケースを 3 通りから選べます。
...
VFM 2.7.0 から、プロセッサ構築の直前に呼ばれる `editPlugins(plugins)` フックが追加されました。
